...
Goal 创建、显式激活、Run 请求和 Approval 决策调用统一的 Application 用例；请求携带的
幂等键与规范化请求摘要绑定，冲突返回 `409`。错误响应中的 `audit://` 仍不是已持久化证据，
配置/改进接口也尚未完成 Consumer Contract，因此 API 仍属于工程预览。API 不直接执行
Action，持久执行由 Temporal Worker 通过相同 Application 用例推进。
