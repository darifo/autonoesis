...
from autonoesis_domain import CandidateStatus, CandidateVersion, Release
...
class CandidateLifecycleService:
    ...
    async def promote(
        ...
        candidate = await self._repository.get_candidate(identity.tenant_id, candidate_id)
        candidate = candidate.transition_to(CandidateStatus.STABLE)
        release = Release(
    ...
    async def rollback(self, identity: IdentityContext, release_id: UUID) -> Release:
        try:
            ...
            raise
        candidate = await self._repository.get_candidate(identity.tenant_id, release.candidate_id)
        await self._repository.save_candidate(candidate.transition_to(CandidateStatus.ROLLED_BACK))
        return Release(
            tenant_id=identity.tenant_id,
