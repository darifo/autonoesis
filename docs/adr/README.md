# Architecture Decision Records

ADRs record decisions affecting authoritative ownership, process boundaries, protocols, security, tenant isolation, and release governance that are hard to reverse. Accepted ADRs are not modified except for textual clarification; changing a decision requires a new ADR declaring the supersession relationship.

## Index

- [ADR-0000: Record Architecture Decisions](0000-record-architecture-decisions.md)
- [ADR-0001: Use Git Monorepo](0001-use-one-git-monorepo.md)
- [ADR-0002: Separate Business State from Durable Execution](0002-separate-state-from-execution.md)
- [ADR-0003: Goal-First Kernel with Capability Pack Extension](0003-goal-first-capability-pack-platform.md)
- [ADR-0004: Deterministic Workflow First, Agent Loop Limited to Bounded Tasks](0004-deterministic-workflow-first.md)
- [ADR-0005: All External Side Effects Through Unified Tool Gateway](0005-unified-tool-gateway.md)
- [ADR-0006: Outbox/Inbox for Event Delivery](0006-outbox-inbox-event-delivery.md)
- [ADR-0007: Separate State, Environment, Knowledge, Memory, and Context](0007-separate-state-environment-knowledge-memory-context.md)
- [ADR-0008: Evaluation-First with Separation of Improvement, Grading, and Release](0008-evaluation-first-separation.md)
- [ADR-0009: Capability Pack as Standard Extension Unit](0009-capability-pack-extension-unit.md)
- [ADR-0010: Multi-Dimensional Tenant Isolation and Execution-Time Authorization](0010-multi-tenant-isolation-runtime-authz.md)
- [ADR-0011: Candidate → Evaluation → Approval → Shadow → Canary → Stable Release Pipeline](0011-candidate-release-pipeline.md)
- [ADR-0012: Freeze Governed Execution Domain Contracts Before Persistence](0012-freeze-governed-execution-domain-contracts.md)
- [ADR-0013: Make PostgreSQL the Process-Independent Business Authority](0013-postgresql-authority-and-role-separation.md)
- [ADR-0014: Make Application Use Cases the Governed Execution Writer](0014-application-owned-execution-transactions.md)
- [ADR-0015: Make Tool Execution an Atomic Governed Boundary](0015-atomic-governed-tool-execution.md)
- [ADR-0016: Make Temporal Run Orchestration Recoverable and Deterministic](0016-durable-temporal-run-orchestration.md)
- [ADR-0017: Require Immutable Evidence and Independent Outcome Verification](0017-trusted-evidence-outcome-audit-chain.md)
